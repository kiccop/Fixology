...
/**
 * Component for official Strava branding using embedded SVGs.
 * This avoids external image loading issues and ensures perfect rendering.
 */
export function StravaLogo({ variant = 'powered-by', className = '', loading = false, onClick }: StravaLogoProps) {
    // Strava Official Orange: #FC4C02

    // The official Strava 'M' mark (two triangles)
    const StravaMark = () => (
    ...
    if (variant === 'connect-button') {
        return (
            <div
                onClick={onClick}
                className={`inline-flex items-center bg-[#FC4C02] hover:bg-[#E34402] text-white font-bold py-2 px-5 rounded shadow-lg transition-all cursor-pointer gap-2.5 active:scale-95 ${className}`}
            >
